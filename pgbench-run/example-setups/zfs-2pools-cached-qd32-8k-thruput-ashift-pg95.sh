...
zpool create fastpool01 -o ashift=13 \
...
zpool create fastpool02 -o ashift=13 \
...
# create zfs filesystem for databases (no atime, 8k blocks, zil for thruput always) 
zfs create -o atime=off -o recordsize=8k -o logbias=throughput fastpool01/db01

# create zfs filesystem for xlogs (no atime, 8k blocks, zil for thruput since no slog dev)
zfs create -o atime=off -o recordsize=8k -o logbias=throughput fastpool02/xlog01
...
runuser -l postgres -c "/usr/pgsql-9.5/bin/initdb -D /fastpool01/db01/test -X /fastpool02/xlog01/test"
runuser -l postgres -c "/usr/pgsql-9.5/bin/pg_ctl -D /fastpool01/db01/test start"
...
/tmp/pgbench-run.sh -c 10 -t 1000 -r 100 -d test -i zfs-2pools-cached-qd32-8k-thruput-ashift-pg95 -o /tmp/pgbench.out
